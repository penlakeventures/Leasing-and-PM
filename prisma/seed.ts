...
async function main() {
  ...
  for (const p of projects) {
    ...
    if (existingUnits > 0) {
      console.log(`  → ${p.internalName} already has units, skipping.`);
      continue;
    }
    ...
    console.log(`  → ${p.internalName}: ${p.towns} towns, ${p.suites} suites${p.barn ? " + barn" : ""}`);
  }

  console.log("Seeding Alberta deposit interest rate for 2026 (0%, per regulation)…");
